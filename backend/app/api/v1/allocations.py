...
def auto_allocate_seat(
    ...
    """
    Auto-allocates the best available seat to an employee.
    Priority: 1) Same Project floor, 2) Same Department floor, 3) First Available.
    New joiners are placed near their project team when possible.
    """
    ...
    # 1. Try Same Project (place near team)
    if employee.project_id:
        project_colleagues = db.query(Employee).filter(
    ...
    if not best_seat and employee.department_id:
        dept_colleagues = db.query(Employee).filter(
